...
INSERT INTO public.profiles (id, email, name, has_completed_onboarding, has_full_access)
VALUES ('00000000-0000-0000-0000-000000000001', 'test@example.com', 'Test User', true, true)
...
-- ---------------------------------------------------------------------------
-- 10. Diarized transcript fixture (T1 accuracy/diarization E2E)
-- ---------------------------------------------------------------------------
-- Dedicated series + meeting, isolated from every other seed row so this
-- fixture cannot collide with a count-based assertion elsewhere (see
-- e2e/regression/diarized-transcript.spec.ts). The suggestion below is
-- seeded, not generated by a live model, so the owner-attribution render is
-- deterministic in CI without an AI key; the live AssemblyAI extraction path
-- is validated separately in the accuracy spike (see task-11-brief.md).
INSERT INTO public.meeting_series (id, name, description, cadence, default_attendees, owner_id)
VALUES
  ('10000000-0000-0000-0000-000000000004', 'Diarization QA', 'Dedicated fixture for the diarized transcript E2E journey', 'adhoc', ARRAY['Sarah Lee', 'Mike Ross'], '00000000-0000-0000-0000-000000000001')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.meetings (
  id, series_id, sequence_number, title, date, attendees, status, notes_markdown,
  transcript_raw, transcript_diarized, speaker_map, transcript_segments, completed_at
)
VALUES (
  ...
  'Diarized Sync',
  '2026-06-30',
  ARRAY['Sarah Lee', 'Mike Ross'],
  'completed',
  '',
  E'Sarah Lee: We still need the migration reviewed.\nMike Ross: I will take the migration review.',
  true,
  '{"A":"Sarah Lee","B":"Mike Ross"}'::jsonb,
  '[
    {"speaker":"A","start":0,"end":3,"text":"We still need the migration reviewed.","confidence":0.94},
    {"speaker":"B","start":3,"end":6,"text":"I will take the migration review.","confidence":0.92}
  ]'::jsonb,
  now()
)
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.meeting_ai_suggestions (
  id, meeting_id, series_id, type, category, title, owner_name, confidence, source_excerpt, status
)
VALUES (
  ...
  'Mike Ross',
  0.92,
  'I will take the migration review.',
  'pending'
)
ON CONFLICT (id) DO NOTHING;
